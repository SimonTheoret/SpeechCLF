...
# page config
st.set_page_config(
    page_title="Readability prediction",
    page_icon=":books:",
    layout="centered",
...
# image cover
image = Image.open(
    requests.get("https://i.postimg.cc/hv6yfMYz/cover-books.jpg", stream=True).raw
)
st.image(image)

# description
st.write(
    "This app uses deep learning to estimate the reading complexity of a custom text. Enter your text below, and we will run it through one of the two transfomer models and display the result."
)
...
model_name = st.selectbox(
    "Which model would you like to use?", ["DistilBERT", "DistilRoBERTa"]
)

# input text
input_text = st.text_area(
    "Which text would you like to rate?", "Please enter the text in this field."
)
...
if st.button("Compute readability"):
    ...
    if not os.path.isfile(folder_path + "pytorch_model.bin"):
        with st.spinner(
            ...
            urllib.request.urlretrieve(
                weight_path, folder_path + "pytorch_model.bin", show_progress
            )
    ...
    with st.spinner("Computing prediction..."):
        ...
        model = get_model(
            ...
            pretrained=folder_path + "pytorch_model.bin",
        )
        model.eval()
        ...
        st.write(
            "**Note:** readability scores are scaled to [0, 100%]. A higher score means that the text is easier to read."
        )
...
with st.expander("Show example texts"):
    st.table(
        pd.DataFrame(
            {
                "Text": [
                    "A dog sits on the floor. A cat sleeps on the sofa.",
                    "This app does text readability prediction. How cool is that?",
...
with st.expander("Read about the models"):
    st.write(
        "Both transformer models are part of my top-9% solution to the CommonLit Readability Kaggle competition. The pre-trained language models are fine-tuned on 2834 text snippets. [Click here](https://github.com/kozodoi/Kaggle_Readability) to see the source code and read more about the training pipeline."
    )

# metric
with st.expander("Read about the metric"):
    st.write(
        "The readability metric is calculated on the basis of a Bradley-Terry analysis of more than 111,000 pairwise comparisons between excerpts. Teachers spanning grades 3-12 (a majority teaching between grades 6-10) served as the raters for these comparisons. The raw scores vary in [-4, 2] and are scaled to [0, 100%] for convenience. More details on the used reading complexity metric are available [here](https://www.kaggle.com/c/commonlitreadabilityprize/discussion/240886)."
    )
